...
class AbstractTest:
    ...
    def test_camera_btn(self, url):
        ...
        """
        self.driver.get(url)
        WebDriverWait(self.driver, timeout=10).until(expected_conditions.element_to_be_clickable((By.CLASS_NAME, 'camera-icon-background')))
        el = self.driver.find_element_by_class_name('camera-icon-background')
        el.click()
        WebDriverWait(self.driver, timeout=10).until(expected_conditions.presence_of_element_located((By.CLASS_NAME, 'camera-container')))
        assert self.driver.find_element(By.CLASS_NAME, 'camera-container') != 'undefined'

@pytest.mark.usefixtures('init_firefox')
class TestFirefox(AbstractTest):
    """
...
class TestChrome(AbstractTest):
    """
    Run tests on Chrome
    """

@pytest.mark.usefixtures('init_safari_desktop')
class TestSafari(AbstractTest):
    """
    Run tests on Safari
    """
